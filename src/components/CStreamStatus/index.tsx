...
const CStreamStatus = ({ onChange }: CStreamStatusProps) => {
  ...
  return (
    <div className="inline-flex mobile:h-[52px] justify-center items-center rounded-[55px] border-[1px] py-2 px-2 mobile:border-[#0501421A] border-midnightBlue gap-2 cursor-pointer select-none h-[48px]">
      {streamStatuses.map((item) => (
        <div
          key={item.label}
          className={`px-4 py-1 mobile:py-2 mobile:text-base font-medium ${
