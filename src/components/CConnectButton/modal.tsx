...
import CModalSheet from '../CModalSheet';
...
const Modal = ({ open, address, closeModal, isMinimized }: ModalProps) => {
  ...
  return (
    <>
      ...
      <CModalSheet open={open} onClose={closeModal} contentClass="!px-6">
        <div ref={modalRef}>
          <p className="text-2xl font-medium text-midnightBlue">Wallet</p>
          <div className="flex flex-col items-center justify-center gap-2 mt-6">
            <div className="h-12 px-4 rounded-md bg-alabaster text-midnightBlue flex justify-between items-center w-full">
              <span>{shortenAddress(address, 5)}</span>
              <Image src={copy} alt="copy" onClick={handleCopy} />
            </div>
            <Link
